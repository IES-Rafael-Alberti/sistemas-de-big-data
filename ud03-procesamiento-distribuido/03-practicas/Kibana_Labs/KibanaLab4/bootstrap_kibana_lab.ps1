...
$lab = "lab_kibana_observabilidad"
New-Item -Type Directory -Force -Path "$lab/logstash/pipeline","$lab/data","$lab/es","$lab/scripts" | Out-Null
@'
version: "3.9"
services:
  elasticsearch:
...
'@ | Set-Content -Path "$lab/docker-compose.yml"
@'
input {
  file {
    path => "/data/logs.csv"
...
'@ | Set-Content -Path "$lab/logstash/pipeline/logstash.conf"
@'
{
  "index_patterns": [
    "logs-sim-*"
...
'@ | Set-Content -Path "$lab/es/es_index_template.json"
@'
...
'@ | Set-Content -Path "$lab/scripts/devtools_template.md"
@'
#!/usr/bin/env bash
set -euo pipefail
...
echo -e "`nPlantilla cargada."
'@ | Set-Content -Path "$lab/scripts/load_template.sh"
@'
...
'@ | Set-Content -Path "$lab/scripts/append_logs.py"
@'
...
'@ | Set-Content -Path "$lab/README.md"
@'
...
'@ | Set-Content -Path "$lab/data/logs.csv"
@'
...
'@ | Set-Content -Path "$lab/data/logs.ndjson"
Write-Host "Lab creado en $lab"
