...
def main():
    ...
    # Nota: integramos la plantilla SQL y reemplazamos NOMBRE_TABLA por la vista creada
    curated_glob = f"{CURATED_DIR.as_posix()}/anio=*/mes=*/*.parquet"
    header_sql = f"""-- Checks sobre dataset Parquet particionado
INSTALL parquet; LOAD parquet;
CREATE OR REPLACE VIEW sales_curated AS
SELECT * FROM read_parquet('{curated_glob}');
"""

    template_path = BASE / "duckdb_checks_template.sql"
    template_sql = ""
    if template_path.exists():
        template_sql = template_path.read_text(encoding="utf-8")
        # Reemplazar la tabla placeholder por la vista creada
        template_sql = template_sql.replace("NOMBRE_TABLA", "sales_curated")
    else:
        # Fallback mínimo por si no existe la plantilla
        template_sql = """-- Plantilla no encontrada, checks mínimos\nSELECT 'rows' AS rule_id, COUNT(*) AS total FROM sales_curated;\n"""

    checks_sql = header_sql + "\n" + template_sql
